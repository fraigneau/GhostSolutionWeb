...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Function to check and initialize sections
  function checkSections() {
    console.log("Checking sections...");

    // Vérifier si l'élément matrixCanvas existe
    const matrixBg = document.getElementById("matrixCanvas");
    if (!matrixBg) {
      console.error("Matrix canvas not found, creating it");
      const heroSection = document.querySelector(".hero");
      if (heroSection) {
        const newMatrixBg = document.createElement("div");
        newMatrixBg.className = "matrix-bg";
        newMatrixBg.id = "matrixCanvas";
        heroSection.appendChild(newMatrixBg);
      }
    } else {
      console.log("Matrix canvas found");
    }

    // Vérifier les autres sections importantes
  ...
  function initMatrixBackground() {
    const matrixBg = document.getElementById("matrixCanvas");
    if (!matrixBg) {
      console.error("Matrix background not found");
      return;
    }
    ...
    function drawMatrix() {
      ...
      // Continue animation with a slower frame rate (using setTimeout)
      setTimeout(() => {
        requestAnimationFrame(drawMatrix);
      }, 50); // Ajouter un délai de 100ms entre chaque frame
    }
    ...
    window.addEventListener("resize", function () {
      ...
      // Réinitialiser les colonnes après redimensionnement
      const newColumns = Math.floor(canvas.width / 20);

      // Ajuster le nombre de gouttes si nécessaire
