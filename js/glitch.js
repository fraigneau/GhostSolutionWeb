document.addEventListener("DOMContentLoaded", function () {
  ...
  let glitchTimeout;
  let scrollThreshold = 20; // Scroll threshold to start disabling the effect
  let scrollMax = 350; // Maximum scroll to completely disable the effect
  ...
  function handleScroll() {
    ...
    if (scrollPosition !== lastScrollPosition) {
      ...
      noScrollTimer = setTimeout(() => {
        if (autoGlitchActive) {
          console.log(
            "No scroll detected for 10 seconds, disabling glitch effect"
          );
          disableAutoGlitch();
        }
      }, autoDisableDelay);
    ...
    if (scrollPosition > scrollThreshold) {
      ...
      const scrollProgress = Math.min(
        1,
        (scrollPosition - scrollThreshold) / (scrollMax - scrollThreshold)
      );
  ...
  function prepareGlitchText() {
    ...
    const glitchElements = [
      ...document.querySelectorAll(
        "h1, h2, h3, .logo, .service-title, .work-title"
      ),
    ];
  ...
  function startRandomGlitches(intensity = 1) {
    ...
    switch (randomGlitchEffect) {
      case 0:
        ...
        body.style.filter = `hue-rotate(${
          Math.random() * 360 * adjustedIntensity
        }deg)`;
        setTimeout(() => {
          body.style.filter = "";
      ...
      case 1:
        ...
        body.style.transform = `translate(${
          (Math.random() * 10 - 5) * adjustedIntensity
        }px, ${(Math.random() * 10 - 5) * adjustedIntensity}px)`;
        setTimeout(() => {
      ...
      case 2:
        ...
        body.style.filter = `blur(${
          Math.random() * 5 * adjustedIntensity
        }px) contrast(${1 + Math.random() * adjustedIntensity})`;
        setTimeout(() => {
          body.style.filter = "";
    ...
    const nextGlitchTime =
      (2000 + Math.random() * 5000) * (1 / adjustedIntensity);
    glitchTimeout = setTimeout(
      () => startRandomGlitches(intensity),
      nextGlitchTime
    );
